...
class Database {
  ...
  saveMessage(messageData) {
    return new Promise((resolve, reject) => {
      const sql = `
        INSERT INTO messages (userId, userMessage, botResponse, language, timestamp)
        VALUES (?, ?, ?, ?, datetime('now'))
      `;

      this.db.run(sql, [messageData.userId, messageData.userMessage, messageData.botResponse, messageData.language || null], (err) => {
        if (err) {
  ...
  /**
   * Сводка для обнаружения аномалий/злоупотреблений - без IP и без каких-либо
   * персональных данных, только userId (chatId/sessionId), язык, длина
   * сообщений. Данные видны только за то время, что жива текущая "тёплая"
   * функция (БД на /tmp сбрасывается при холодном старте) - это снимок
   * недавней активности, а не полная история.
   */
  getStatsSummary() {
    return new Promise((resolve, reject) => {
      const queries = {
        totals: `SELECT COUNT(*) as totalMessages, COUNT(DISTINCT userId) as uniqueUsers FROM messages`,
        byLanguage: `SELECT COALESCE(language, 'не указан') as language, COUNT(*) as count FROM messages GROUP BY language ORDER BY count DESC`,
        topActive: `SELECT userId, COUNT(*) as count FROM messages GROUP BY userId ORDER BY count DESC LIMIT 5`,
        lengthStats: `SELECT AVG(LENGTH(userMessage)) as avgLength, MAX(LENGTH(userMessage)) as maxLength FROM messages`
      };
      ...
      keys.forEach(key => {
        const method = (key === 'totals' || key === 'lengthStats') ? 'get' : 'all';
        this.db[method](queries[key], (err, data) => {
          if (hasError) return;
          if (err) {
            hasError = true;
            console.error(`❌ Error in getStatsSummary (${key}):`, err);
            return reject(err);
          }
          results[key] = data;
          completed++;
