...
function createTelegramBot({ contentGenerator, database, analytics }) {
  ...
  // Ссылка ведёт на собственный сайт (форма партнёра + чат вместе на одной
  // странице), а не напрямую на партнёрский портал - так пользователь сразу
  // попадает в среду с переводом/объяснениями рядом с формой, а не на голую
  // немецкую форму без контекста.
...
Я Fintik - перевожу и объясняю немецкую автостраховку (Kfz-Versicherung) 🇩🇪
...
📞 Команды:
/портал - сайт со сравнением цен и чатом
...
Там же, рядом с формой, есть чат - если что-то будет непонятно при заполнении, сразу спроси там же, не обязательно возвращаться сюда.
    `, {
      reply_markup: {
        inline_keyboard: [
...
👇 Узнай точную цену для своей машины:
  ...
  async function handleQuestion(msg) {
    ...
    try {
      ...
      await database.saveMessage({ userId: msg.from.id, userMessage, botResponse: answer.text, language: 'ru', timestamp: new Date() });
  ...
  async function handleCallbackQuery(callbackQuery) {
    ...
    await bot.sendMessage(chatId, helpTexts[data] || 'Напиши свой вопрос, помогу! 😊', {
      reply_markup: { inline_keyboard: [[{ text: '🔗 Перейти на сайт', url: portalUrl }]] }
    });
  ...
  console.log('🤖 Telegram bot создан (webhook-режим, Fintik)');
